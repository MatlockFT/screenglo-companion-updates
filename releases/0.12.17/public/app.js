...
function renderDownloads(items) {
  ...
  $('#download-list').innerHTML = items.map(item => {
    ...
    const watchLabel = item.watchState === 'watched' ? 'WATCHED' : item.watchState === 'in_progress' ? `CONTINUE · ${item.watchProgress || 0}%` : 'UNWATCHED';
    const showPlaybackProgress = item.complete && item.watchState === 'in_progress' && item.watchProgress > 0;

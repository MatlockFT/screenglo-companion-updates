...
class LetterboxdService {
  ...
  async listCatalog() {
    if ((!this.state.publicCatalog || Date.now() - this.state.publicCatalog.at >= COLLECTION_TTL) && Date.now() - (this.catalogAttemptAt || 0) >= 15 * 60 * 1000) {
      this.catalogAttemptAt = Date.now();
      try {
        const page = await this.text(`${LETTERBOXD_ORIGIN}/lists/`, 8000);
        const items = parsePublicLists(page.value);
        if (items.length) this.state.publicCatalog = { at: Date.now(), items };
      } catch { /* Keep previously fetched lists or the verified starter catalog. */ }
    }
    const items = this.state.publicCatalog?.items?.length ? this.state.publicCatalog.items : PUBLIC_LIST_STARTERS;
    this.persist();
    return items;
  }
  ...
  async discovery(filter = 'all', limit = 12, seed = '', offset = 0, exclude = null, requestedList = '') {
    ...
    if (requestedList && !listUrl) throw new Error('Use a public Letterboxd list link.');
    let openedList = null;
    if (listUrl) {
...
function recordWatched(value) {
  ...
  if (item.hash) playbackProgress[item.hash.toLowerCase()] = {
    hash: item.hash.toLowerCase(), fileId: item.fileId, position: item.duration, duration: item.duration,
...
function updatePlaybackState(value) {
  currentPlayback = value.active === false ? { active: false, updatedAt: Date.now() } : {
    ...
    hash: String(value.hash || '').toLowerCase().slice(0, 40), fileId: String(value.fileId || '0').slice(0, 32),
    position: Math.max(0, Number(value.position || 0)), duration: Math.max(0, Number(value.duration || 0)), updatedAt: Date.now(),
  };
  if (currentPlayback.active && currentPlayback.hash && currentPlayback.duration > 0) {
    const previous = playbackProgress[currentPlayback.hash] || {};
    playbackProgress[currentPlayback.hash] = {
      hash: currentPlayback.hash, fileId: currentPlayback.fileId,
      position: currentPlayback.position, duration: currentPlayback.duration,
...
async function downloads() {
  ...
  return items.sort((a, b) => (b.added_on || 0) - (a.added_on || 0)).map(x => {
    ...
    return {
      ...
      watchProgress: (() => {
        const saved = playbackProgress[String(x.hash || '').toLowerCase()];
        if (saved?.watched || playbackHistory.some(item => item.hash === x.hash)) return 100;
        return saved?.duration > 0 ? Math.max(0, Math.min(99, Math.round(saved.position / saved.duration * 100))) : 0;
      })(),
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (route === 'api/letterboxd/lists') return json(res, 200, { ok: true, items: await letterboxd.listCatalog() });

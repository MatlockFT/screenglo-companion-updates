...
let publicListsItems = [], publicListsHasMore = false, publicListsTotal = 0, publicListsLoading = false, publicListsSeed = String(Date.now());
...
function renderDownloads(items) {
  ...
  $('#download-list').innerHTML = items.map(item => {
    ...
    const showPlaybackProgress = item.complete && item.watchProgress > 0 && item.watchProgress < 100;
    const watchLabel = item.watchState === 'watched'
      ? `WATCHED${showPlaybackProgress ? ` · ${item.watchProgress}%` : ''}`
      : showPlaybackProgress ? `CONTINUE · ${item.watchProgress}%` : 'UNWATCHED';
    return `<article class="download ${item.complete ? 'complete' : ''}" data-download-id="${escape(item.id)}">
      <div class="download-head">${item.complete ? '<img class="library-art" alt="" hidden>' : ''}<div><h2>${escape(item.title)}</h2><div class="meta"><strong>${escape(item.complete ? watchLabel : item.status)}</strong>${item.complete ? '' : ' · ' + item.progress + '% · ' + escape(item.speed)}</div></div></div>
      ${!item.complete || showPlaybackProgress ? `<div class="track ${showPlaybackProgress ? 'watch-track' : ''}"><div class="fill" style="width:${showPlaybackProgress ? item.watchProgress : item.progress}%"></div></div>` : ''}
...
function savedByText(item) {
  ...
  return item.sources?.[0] || 'COLLECTION';
}
...
async function loadForUs(force = false, append = false) {
  if (letterboxdFilter === 'curated' && !publicListUrl) return showPublicLists(true, force);
...
async function showPublicLists(reset = true, refresh = false) {
  if (publicListsLoading) return;
  publicListsLoading = true;
  ...
  $('#public-list-more').classList.remove('hidden'); $('#public-list-more').disabled = true;
  if (reset) $('#letterboxd-status').textContent = refresh ? 'Refreshing collections…' : 'Loading collections…';
  else $('#letterboxd-status').textContent = 'Finding more collections…';
  try {
    if (refresh) publicListsSeed = String(Date.now());
    const offset = reset ? 0 : publicListsItems.length;
    const data = await api(`letterboxd/lists?limit=12&offset=${offset}&seed=${encodeURIComponent(publicListsSeed)}${refresh ? '&refresh=1' : ''}`);
    publicListsItems = reset ? data.items : [...publicListsItems, ...data.items].filter((item, index, all) => all.findIndex(candidate => candidate.url === item.url) === index);
    publicListsHasMore = Boolean(data.hasMore); publicListsTotal = Number(data.total || publicListsItems.length);
    $('#letterboxd-status').textContent = `${publicListsItems.length} of ${publicListsTotal} live collections`;
    $('#public-list-catalog').innerHTML = publicListsItems.map(item => `<button type="button" data-public-list="${escape(item.url)}"><strong>${escape(item.title)}</strong><small>${escape(item.owner)}</small></button>`).join('');
    ...
    publicListsLoading = false; $('#public-list-more').disabled = false;
    $('#public-list-more').classList.toggle('hidden', !publicListsHasMore);
  }
}
function openPublicList(url) {
  if (letterboxdLoading) return;
  publicListUrl = url; letterboxdItems = [];
  $('#public-list-catalog').classList.add('hidden'); $('#public-list-more').classList.add('hidden'); $('#back-to-lists').classList.remove('hidden');
  loadForUs();
}
$('#back-to-lists').addEventListener('click', () => {
  if (letterboxdLoading) return;
  publicListUrl = '';
  $('#letterboxd-list').replaceChildren();
  $('#letterboxd-more').classList.add('hidden');
  $('#back-to-lists').classList.add('hidden');
  $('#public-list-catalog').classList.remove('hidden');
  $('#public-list-more').classList.toggle('hidden', !publicListsHasMore);
  $('#letterboxd-status').textContent = `${publicListsItems.length} of ${publicListsTotal} live collections`;
});
$('#public-list-more').addEventListener('click', () => showPublicLists(false));

...
const PUBLIC_CATALOG_TTL = 6 * 60 * 60 * 1000;
const TMDB_COLLECTION_TTL = 3 * 60 * 60 * 1000;
const politePause = (milliseconds = 220) => new Promise(resolve => setTimeout(resolve, milliseconds));
const PUBLIC_LIST_STARTERS = [
  ['official', 'letterboxds-top-500-films', "Letterboxd’s Top 500 Films"],
  ['fcbarcelona', 'movies-everyone-should-watch-at-least-once', 'Movies everyone should watch at least once'],
  ['crew', '20-action-packed-creature-features', '20 action-packed creature features'],
  ['colintroy', 'movies-with-treacherous-bridge-crossings', 'Movies with Treacherous Bridge Crossings'],
  ['gracelessbeast', 'this-is-the-world-we-live-in', 'This is the world we live in'],
  ['filmfestival', 'telluride-film-festival-2026', 'Telluride Film Festival 2026'],
].map(([owner, slug, title]) => ({ owner, title, url: `${LETTERBOXD_ORIGIN}/${owner}/list/${slug}/` }));
const PUBLIC_CATALOG_SOURCES = [
  `${LETTERBOXD_ORIGIN}/lists/popular/this/week/`,
  `${LETTERBOXD_ORIGIN}/lists/popular/this/month/`,
  `${LETTERBOXD_ORIGIN}/lists/featured/`,
  `${LETTERBOXD_ORIGIN}/official/lists/`,
  `${LETTERBOXD_ORIGIN}/official/lists/page/2/`,
];
const TMDB_COLLECTIONS = [
  { url: 'tmdb:trending-week', title: 'Trending This Week', owner: 'TMDB · weekly', path: 'trending/movie/week' },
  { url: 'tmdb:popular-now', title: 'Popular Right Now', owner: 'TMDB · daily', path: 'movie/popular' },
  { url: 'tmdb:now-playing', title: 'New in Theaters', owner: 'TMDB · current', path: 'movie/now_playing', region: 'US' },
  { url: 'tmdb:highly-rated', title: 'Highly Rated', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '1000', 'vote_average.gte': '7.2' } },
  { url: 'tmdb:short-and-great', title: 'Great Movies Under 100 Minutes', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '400', 'vote_average.gte': '6.8', 'with_runtime.lte': '100' } },
  { url: 'tmdb:hidden-gems', title: 'Hidden Gems', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '100', 'vote_count.lte': '2500', 'vote_average.gte': '7' } },
  { url: 'tmdb:recent-gems', title: 'Recent Gems', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '250', 'vote_average.gte': '6.8', recentYears: '3' } },
  { url: 'tmdb:horror', title: 'Horror Night', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '500', 'vote_average.gte': '6.5', with_genres: '27' } },
  { url: 'tmdb:comedy', title: 'Actually Good Comedies', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '750', 'vote_average.gte': '6.5', with_genres: '35' } },
  { url: 'tmdb:science-fiction', title: 'Science Fiction', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '600', 'vote_average.gte': '6.7', with_genres: '878' } },
  { url: 'tmdb:classics', title: 'Essential Classics', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '1000', 'vote_average.gte': '7', 'primary_release_date.lte': '1999-12-31' } },
  { url: 'tmdb:world-cinema', title: 'World Cinema', owner: 'TMDB · refreshed', path: 'discover/movie', query: { sort_by: 'vote_average.desc', 'vote_count.gte': '300', 'vote_average.gte': '7', without_original_language: 'en' } },
];

function parsePublicLists(html) {
  const lists = new Map();
  for (const match of String(html).matchAll(/<a\b[^>]*href="([^"]*\/list\/[^"]+)"[^>]*>([\s\S]*?)<\/a>/gi)) {
    let url; try { url = publicListUrl(new URL(decodeHtml(match[1]), LETTERBOXD_ORIGIN).toString()); } catch { continue; }
    const title = decodeHtml(match[2].replace(/<[^>]*>/g, '')).trim();
    if (url && title.length > 3 && !/^\d+[KkMm]?$/.test(title) && !lists.has(url)) lists.set(url, { url, title, owner: new URL(url).pathname.split('/')[1] });
  }
  return [...lists.values()].slice(0, 40);
}
...
function publicListUrl(value) {
  try {
    ...
    if (url.protocol !== 'https:' || url.port || url.username || url.password || !/^(?:www\.)?letterboxd\.com$/i.test(url.hostname)) return '';
    return /^\/[a-z0-9_]+\/list\/[a-z0-9-]+\/?$/i.test(url.pathname) ? `${LETTERBOXD_ORIGIN}${url.pathname.replace(/\/?$/, '/')}` : '';
  } catch { return ''; }
}

function emptyState() {
  return { version: CACHE_VERSION, updatedAt: 0, users: {}, curated: [], community: {}, tmdb: {}, sourceHealth: {} };
}

function seededValue(value, seed) {
  let hash = 2166136261;
  for (const character of `${seed}:${value}`) { hash ^= character.charCodeAt(0); hash = Math.imul(hash, 16777619); }
  return hash >>> 0;
}

function orderRecommendations(candidates, filter = 'all', seed = '') {
  const ordered = values => [...values].sort((left, right) => seededValue(left.key, seed) - seededValue(right.key, seed));
  if (filter !== 'all') return ordered(candidates);
  const pools = [
    ordered(candidates.filter(item => item.savedBy.length === 2)),
    ordered(candidates.filter(item => item.curatedBy.length)),
    ordered(candidates.filter(item => item.savedBy.length === 1 && item.savedBy.includes('tyler'))),
    ordered(candidates.filter(item => item.savedBy.length === 1 && item.savedBy.includes('gloria'))),
  ];
  const result = [], seen = new Set();
  while (pools.some(pool => pool.length)) {
    for (const pool of pools) {
      const next = pool.shift();
      if (next && !seen.has(next.key)) { seen.add(next.key); result.push(next); }
    }
  }
  return result;
}
...
class LetterboxdService {
  constructor({ dataDir, getConfig, fetchImpl = fetch }) {
    ...
    this.openLists = new Map();
    this.tmdbCollections = new Map();
  ...
  status() {
    ...
    return {
      ...
      degraded: Object.values(this.state.sourceHealth || {}).some(source => source?.ok === false),
      issues: Object.entries(this.state.sourceHealth || {}).filter(([, source]) => source?.ok === false).map(([name]) => name),
    };
  }
  ...
  async sync(force = false) {
    ...
    this.syncPromise = (async () => {
      ...
      let tyler = this.state.users.tyler;
      let gloria = this.state.users.gloria;
      const sourceHealth = { ...(this.state.sourceHealth || {}) };
      try { tyler = await this.member('Tyler', tylerName); sourceHealth.tyler = { ok: true, checkedAt: Date.now() }; }
      catch (error) { sourceHealth.tyler = { ok: false, checkedAt: Date.now(), message: String(error.message || 'Unavailable').slice(0, 160) }; }
      await politePause(500);
      try { gloria = await this.member('Gloria', gloriaName); sourceHealth.gloria = { ok: true, checkedAt: Date.now() }; }
      catch (error) { sourceHealth.gloria = { ok: false, checkedAt: Date.now(), message: String(error.message || 'Unavailable').slice(0, 160) }; }
      const curated = [];
      for (const candidate of config.letterboxdLists || []) {
        const listUrl = publicListUrl(candidate);
        if (!listUrl) continue;
        const healthKey = `list:${listUrl}`;
        try { curated.push({ url: listUrl, items: await this.collection(listUrl) }); sourceHealth[healthKey] = { ok: true, checkedAt: Date.now() }; }
        catch (error) {
          const cached = this.state.curated.find(list => list.url === listUrl);
          if (cached) curated.push(cached);
          sourceHealth[healthKey] = { ok: false, checkedAt: Date.now(), message: String(error.message || 'Unavailable').slice(0, 160) };
        }
      }
      if (!tyler && !gloria && !curated.length) throw new Error('No Letterboxd source has completed its first sync yet.');
      this.state = { ...this.state, version: CACHE_VERSION, updatedAt: Date.now(), users: { tyler, gloria }, curated, sourceHealth };
      this.persist();
      return this.status();
  ...
  async listCatalog({ offset = 0, limit = 12, refresh = false, seed = '' } = {}) {
    const catalogStale = !this.state.publicCatalog || Date.now() - this.state.publicCatalog.at >= PUBLIC_CATALOG_TTL;
    if ((refresh || catalogStale) && (refresh || Date.now() - (this.catalogAttemptAt || 0) >= 15 * 60 * 1000)) {
      this.catalogAttemptAt = Date.now();
      const discovered = new Map();
      for (const source of PUBLIC_CATALOG_SOURCES) {
        try {
          const page = await this.text(source, 8000);
          for (const item of parsePublicLists(page.value)) if (!discovered.has(item.url)) discovered.set(item.url, item);
          await politePause(180);
        } catch { /* One directory can fail without discarding the others. */ }
      }
      if (discovered.size) this.state.publicCatalog = { at: Date.now(), items: [...discovered.values()].slice(0, 120) };
    }
    const letterboxdItems = this.state.publicCatalog?.items?.length ? this.state.publicCatalog.items : PUBLIC_LIST_STARTERS;
    const tmdbItems = this.getConfig().tmdbToken ? TMDB_COLLECTIONS.map(({ url, title, owner }) => ({ url, title, owner })) : [];
    const rotation = seed || new Date().toISOString().slice(0, 10);
    const rotatingLetterboxd = [...letterboxdItems].sort((left, right) => seededValue(left.url, rotation) - seededValue(right.url, rotation));
    const items = [];
    const max = Math.max(tmdbItems.length, rotatingLetterboxd.length);
    for (let index = 0; index < max; index++) {
      if (tmdbItems[index]) items.push(tmdbItems[index]);
      if (rotatingLetterboxd[index]) items.push(rotatingLetterboxd[index]);
    }
    const pageOffset = Math.max(0, Math.min(500, Number(offset) || 0));
    const pageSize = Math.max(1, Math.min(24, Number(limit) || 12));
    this.persist();
    return { items: items.slice(pageOffset, pageOffset + pageSize), total: items.length, offset: pageOffset, hasMore: pageOffset + pageSize < items.length, refreshedAt: this.state.publicCatalog?.at || null };
  }

  async tmdbCollection(collection, limit, offset, exclude = null) {
    const token = this.getConfig().tmdbToken;
    if (!token) throw new Error('Add the TMDB token in Settings to open this collection.');
    const cacheKey = collection.url;
    const previous = this.tmdbCollections.get(cacheKey);
    let cached = previous && Date.now() - previous.at < TMDB_COLLECTION_TTL
      ? previous : { at: Date.now(), items: [], total: 0, nextPage: 1, exhausted: false };
    const desired = Math.min(200, Math.max(offset + limit + 12, 40));
    try {
      while (cached.items.length < desired && !cached.exhausted && cached.nextPage <= 10) {
        const page = cached.nextPage;
        const target = new URL(`https://api.themoviedb.org/3/${collection.path}`);
        target.searchParams.set('language', 'en-US');
        target.searchParams.set('include_adult', 'false');
        target.searchParams.set('page', String(page));
        if (collection.region) target.searchParams.set('region', collection.region);
        for (const [name, rawValue] of Object.entries(collection.query || {})) {
          if (name === 'recentYears') {
            const date = new Date(); date.setUTCFullYear(date.getUTCFullYear() - Number(rawValue || 3));
            target.searchParams.set('primary_release_date.gte', date.toISOString().slice(0, 10));
          } else if (name === 'without_original_language') {
            // TMDB has no inverse language filter, so this is applied after retrieval.
          } else target.searchParams.set(name, String(rawValue));
        }
        const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), 12_000);
        try {
          const response = await this.fetchImpl(target, { signal: controller.signal, headers: { authorization: `Bearer ${token}`, accept: 'application/json' } });
          if (!response.ok) throw new Error(`TMDB returned ${response.status}.`);
          const payload = await response.json();
          cached.total = Math.min(200, Number(payload.total_results || 0));
          for (const movie of payload.results || []) {
            if (!movie?.id || !movie?.title || movie.adult) continue;
            if (collection.query?.without_original_language && movie.original_language === collection.query.without_original_language) continue;
            if (cached.items.some(item => item.tmdbId === String(movie.id))) continue;
            cached.items.push({
              title: String(movie.title), year: String(movie.release_date || '').slice(0, 4), slug: '', url: `https://www.themoviedb.org/movie/${movie.id}`,
              tmdbId: String(movie.id), posterUrl: movie.poster_path ? `https://image.tmdb.org/t/p/w342${movie.poster_path}` : '',
              overview: String(movie.overview || ''), savedBy: [], curated: true, sources: [collection.title], communityRating: null,
            });
          }
          cached.nextPage = page + 1;
          cached.exhausted = !payload.results?.length || cached.nextPage > Math.min(10, Number(payload.total_pages || 10));
          this.tmdbCollections.set(cacheKey, cached);
        } finally { clearTimeout(timer); }
      }
    } catch (error) {
      if (previous?.items?.length) cached = previous;
      else if (!cached.items.length) throw error;
    }
    const watched = item => this.state.users.tyler?.watched?.[normalizeFilmKey(item.title, item.year)] || this.state.users.gloria?.watched?.[normalizeFilmKey(item.title, item.year)] || exclude?.(item);
    const available = cached.items.filter(item => !watched(item));
    const selected = available.slice(offset, offset + limit);
    const hasMore = offset + selected.length < available.length || !cached.exhausted && cached.nextPage <= 10;
    const total = available.length + (cached.exhausted ? 0 : Math.max(1, cached.total - cached.items.length));
    return { status: this.status(), total, offset, hasMore, items: selected };
  }

  async discovery(filter = 'all', limit = 12, seed = '', offset = 0, exclude = null, requestedList = '') {
    const tmdbList = TMDB_COLLECTIONS.find(collection => collection.url === requestedList);
    const listUrl = requestedList ? publicListUrl(requestedList) : '';
    if (requestedList && !listUrl && !tmdbList) throw new Error('That collection is unavailable. Refresh Collections and try again.');
    if (tmdbList) return this.tmdbCollection(tmdbList, Math.max(1, Math.min(30, Number(limit) || 12)), Math.max(0, Math.min(5000, Number(offset) || 0)), exclude);
    let openedList = null;
    if (listUrl) {
      let cached = this.openLists.get(listUrl);
      if (!cached || Date.now() - cached.at >= COLLECTION_TTL) {
        const items = await this.collection(listUrl);
        cached = { at: Date.now(), items };
        if (this.openLists.size >= 8) this.openLists.delete(this.openLists.keys().next().value);
        this.openLists.set(listUrl, cached);
      }
      openedList = cached.items;
      this.state.openedLists = [...new Set([listUrl, ...(this.state.openedLists || [])])].slice(0, 20);
    }
    if (!listUrl && !this.state.updatedAt) {
      this.sync().catch(() => {});
    ...
    if (openedList) {
      combined.clear();
      Object.values(openedList).forEach(item => add(item, '', listUrl));
    }
    let candidates = [...combined.values()].filter(item => listUrl || (!this.state.users.tyler?.watched?.[item.key] && !this.state.users.gloria?.watched?.[item.key]));
    if (filter === 'both') candidates = candidates.filter(item => item.savedBy.length === 2);
    if (filter === 'tyler') candidates = candidates.filter(item => item.savedBy.length === 1 && item.savedBy.includes('tyler'));
    if (filter === 'gloria') candidates = candidates.filter(item => item.savedBy.length === 1 && item.savedBy.includes('gloria'));
    if (filter === 'curated') candidates = candidates.filter(item => item.curatedBy.length);
    if (exclude && !listUrl) candidates = candidates.filter(item => !exclude(item));
    const rotation = seed || new Date().toISOString().slice(0, 10);
    const pageSize = Math.max(1, Math.min(30, Number(limit) || 12));
    const pageOffset = Math.max(0, Math.min(5000, Number(offset) || 0));
    const ordered = listUrl ? candidates : orderRecommendations(candidates, filter, rotation);
    const selected = ordered.slice(pageOffset, pageOffset + pageSize);
    ...
    return {
      ...
      offset: pageOffset,
      hasMore: pageOffset + selected.length < ordered.length,
      items: detailed.map(({ item, metadata, community }) => ({
        ...
        sources: item.curatedBy.map(value => value.match(/\/list\/([^/]+)/)?.[1]?.replaceAll('-', ' ') || 'Curated list'),
        communityRating: community.rating,
      })),
    };
  }
...
function normalizeLetterboxdConfig(value = {}) {
  const lists = String(value.letterboxdLists || '').split(/[\n,]+/).map(publicListUrl).filter(Boolean);
  return {
    ...
    letterboxdLists: [...new Set(lists)].slice(0, 8),
  };
}
...
const PLAYBACK_HISTORY_FILE = path.join(DATA_DIR, 'playback-history.json');
const PLAYBACK_PROGRESS_FILE = path.join(DATA_DIR, 'playback-progress.json');
...
let downloadSnapshotPromise = null;
const liveClients = new Set();
let liveMonitorTimer = null;
let liveMonitorRunning = false;
let lastLiveDownloadPayload = '';
let playbackHistory = loadPlaybackHistory();
let playbackProgress = loadPlaybackProgress();
let currentPlayback = { active: false };
...
function loadPlaybackHistory() {
  try {
    const parsed = JSON.parse(fs.readFileSync(PLAYBACK_HISTORY_FILE, 'utf8'));
    return Array.isArray(parsed.items) ? parsed.items.slice(0, 500) : [];
  } catch { return []; }
}

function savePlaybackHistory() {
  playbackHistory = playbackHistory.sort((left, right) => Number(right.watchedAt || 0) - Number(left.watchedAt || 0)).slice(0, 500);
  fs.writeFileSync(PLAYBACK_HISTORY_FILE, JSON.stringify({ version: 1, items: playbackHistory }, null, 2));
}

function loadPlaybackProgress() {
  try {
    const parsed = JSON.parse(fs.readFileSync(PLAYBACK_PROGRESS_FILE, 'utf8'));
    return parsed && typeof parsed.items === 'object' && !Array.isArray(parsed.items) ? parsed.items : {};
  } catch { return {}; }
}
...
function savePlaybackProgress() {
  const entries = Object.entries(playbackProgress)
    ...
    .slice(0, 500);
  playbackProgress = Object.fromEntries(entries);
  fs.writeFileSync(PLAYBACK_PROGRESS_FILE, JSON.stringify({ version: 1, items: playbackProgress }, null, 2));
}

function normalizeHistoryTitle(value) {
  return cleanTitle(String(value || '')).normalize('NFKD').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
}

function historyKey(value) {
  const tmdbId = String(value.tmdbId || '').trim();
  if (tmdbId) return `tmdb:${tmdbId}:${String(value.fileId || '0')}`;
  return `${cleanTitle(value.title || value.seriesTitle || '').toLowerCase()}:${String(value.year || '')}:${String(value.fileId || '0')}`;
}

function recordWatched(value) {
  const title = String(value.title || '').trim().slice(0, 240);
  if (!title) throw new Error('A watched title is required.');
  const item = {
    key: historyKey(value), hash: String(value.hash || '').slice(0, 40), fileId: String(value.fileId || '0').slice(0, 32),
    title, seriesTitle: String(value.seriesTitle || '').slice(0, 240), year: String(value.year || '').slice(0, 8),
    tmdbId: String(value.tmdbId || '').slice(0, 24), posterUrl: String(value.posterUrl || '').slice(0, 1000),
    duration: Math.max(0, Number(value.duration || 0)), watchedAt: Date.now(),
  };
  const previous = playbackHistory.find(existing => existing.key === item.key);
  item.playCount = Number(previous?.playCount || 0) + 1;
  playbackHistory = [item, ...playbackHistory.filter(existing => existing.key !== item.key)];
  if (item.hash) {
    const hash = item.hash.toLowerCase();
    const previousProgress = playbackProgress[hash] || {};
    playbackProgress[hash] = {
      hash, fileId: item.fileId,
      position: Math.max(0, Number(previousProgress.position || 0)),
      duration: Math.max(item.duration, Number(previousProgress.duration || 0)),
      watched: true, updatedAt: Date.now(),
    };
  }
  savePlaybackProgress();
  savePlaybackHistory();
  broadcastEvent('history', { ok: true, items: playbackHistory.slice(0, 100) });
  return item;
}

function updatePlaybackState(value) {
  const hash = String(value.hash || '').toLowerCase().slice(0, 40);
  const fileId = String(value.fileId || '0').slice(0, 32);
  const position = Math.max(0, Number(value.position || 0));
  const duration = Math.max(0, Number(value.duration || 0));
  if (hash && duration > 0) {
    const previous = playbackProgress[hash] || {};
    playbackProgress[hash] = {
      hash, fileId, position, duration,
      watched: Boolean(previous.watched), updatedAt: Date.now(),
    };
    savePlaybackProgress();
  }
  currentPlayback = value.active === false ? { active: false, updatedAt: Date.now() } : {
    active: true, playing: Boolean(value.playing), title: String(value.title || '').slice(0, 240),
    episode: String(value.episode || '').slice(0, 240), posterUrl: String(value.posterUrl || '').slice(0, 1000),
    hash, fileId, position, duration, updatedAt: Date.now(),
  };
  broadcastEvent('playback', { ok: true, playback: currentPlayback });
...
async function searchCatalog(query, sort, quality = 'any') {
  ...
  const season = String(query).match(/\bS(\d{1,2})(?:E(\d{1,3}))?\b/i);
  ...
  }).filter(item => quality === 'any' || (quality === '4k' ? /\b(2160p|4k)\b/i.test(item.title) : /\b1080p\b/i.test(item.title) && !/\b(2160p|4k)\b/i.test(item.title))).sort((left, right) => {
    if (!season) return 0;
    const wanted = `S${season[1].padStart(2, '0')}${season[2] ? `E${season[2].padStart(2, '0')}` : ''}`;
    const score = item => String(item.title).toUpperCase().includes(wanted) ? 2 : String(item.title).toUpperCase().includes(`S${season[1].padStart(2, '0')}`) ? 1 : 0;
...
async function metadataFor(item) {
  ...
  const title = item.year || item.tmdbId ? String(item.title || '').trim() : cleanTitle(String(item.title || '')).replace(/\s+\(?\b(?:19|20)\d{2}\b.*$/, '').replace(/\s+S\d{1,2}(?:E\d{1,3})?\b.*$/i, '').trim();
  const year = String(item.year || [...String(item.title || '').matchAll(/\b(?:19|20)\d{2}\b/g)].at(-1)?.[0] || '').slice(0, 4);
  const mediaType = item.mediaType === 'tv' || item.mediaType === 'TV' ? 'tv' : 'movie';
  const key = JSON.stringify([item.tmdbId || '', title.toLowerCase(), year, mediaType]);
  const cached = movieMetadataCache.get(key);
  if (cached && cached.expires > Date.now()) return cached.value;
  const headers = { authorization: `Bearer ${config.tmdbToken}`, accept: 'application/json' };
  let match = /^\d+$/.test(String(item.tmdbId || '')) ? { id: item.tmdbId, media_type: mediaType } : null;
  if (!match) {
    const found = await fetchJson(`https://api.themoviedb.org/3/search/multi?query=${encodeURIComponent(title)}&include_adult=false&language=en-US&page=1`, { headers });
    const normalize = value => String(value || '').normalize('NFKD').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
    const candidates = (found.results || []).filter(x => x.media_type === 'movie' || x.media_type === 'tv');
    const score = x => (normalize(x.title || x.name) === normalize(title) ? 4 : 0) + (year && String(x.release_date || x.first_air_date).startsWith(year) ? 2 : 0);
    match = candidates.sort((a, b) => score(b) - score(a))[0];
    // Avoid decorating a release with an unrelated movie simply because it was the first result.
    if (match && score(match) < 4) return null;
    if (match?.media_type === 'movie' && year && match.release_date && Math.abs(Number(match.release_date.slice(0, 4)) - Number(year)) > 1) return null;
  }
  if (!match) return null;
  const type = match.media_type;
  const details = await fetchJson(`https://api.themoviedb.org/3/${type}/${match.id}?language=en-US&append_to_response=videos`, { headers });
  const seen = new Set();
  const trailers = (details.videos?.results || []).filter(x => x.site === 'YouTube' && /^[a-zA-Z0-9_-]{11}$/.test(x.key) && ['Trailer', 'Teaser'].includes(x.type))
    .sort((a, b) => (Number(b.type === 'Trailer') * 4 + Number(b.official === true) * 2) - (Number(a.type === 'Trailer') * 4 + Number(a.official === true) * 2))
    .filter(x => !seen.has(x.key) && seen.add(x.key)).slice(0, 5).map(x => ({ youtubeId: x.key, name: x.name || 'Trailer' }));
  const value = { tmdbId: String(details.id || match.id), title: details.title || details.name, year: String(details.release_date || details.first_air_date || '').slice(0, 4), mediaType: type === 'tv' ? 'TV' : 'Movie', runtime: details.runtime || details.episode_run_time?.[0] || null, genres: (details.genres || []).map(x => x.name), overview: details.overview || '', posterUrl: details.poster_path ? `https://image.tmdb.org/t/p/w500${details.poster_path}` : '', backdropUrl: details.backdrop_path ? `https://image.tmdb.org/t/p/w1280${details.backdrop_path}` : '', trailers };
  movieMetadataCache.set(key, { value, expires: Date.now() + 6 * 60 * 60 * 1000 });
  while (movieMetadataCache.size > 150) movieMetadataCache.delete(movieMetadataCache.keys().next().value);
  return value;
}
...
async function downloads() {
  ...
  return items.sort((a, b) => (b.added_on || 0) - (a.added_on || 0)).map(x => {
    ...
    return {
      ...
      watchState: (() => {
        const saved = playbackProgress[String(x.hash || '').toLowerCase()];
        if (saved?.watched || playbackHistory.some(item => item.hash === x.hash)) return 'watched';
        return saved?.position > 30_000 && saved?.duration > 0 ? 'in_progress' : 'unwatched';
      })(),
      watchProgress: (() => {
        const saved = playbackProgress[String(x.hash || '').toLowerCase()];
        return saved?.duration > 0 ? Math.max(0, Math.min(100, Math.round(saved.position / saved.duration * 100))) : 0;
      })(),
      resumePosition: Number(playbackProgress[String(x.hash || '').toLowerCase()]?.position || 0),
      playbackDuration: Number(playbackProgress[String(x.hash || '').toLowerCase()]?.duration || 0),
    };
  });
}
async function downloadSnapshot() {
  if (downloadSnapshotPromise) return downloadSnapshotPromise;
  downloadSnapshotPromise = refreshDownloadSnapshot();
  try { return await downloadSnapshotPromise; }
  finally { downloadSnapshotPromise = null; }
}

async function refreshDownloadSnapshot() {
  try {
...
function broadcastEvent(event, value) {
  for (const client of [...liveClients]) {
    try { writeEvent(client, event, value); } catch { liveClients.delete(client); }
  }
}

function scheduleLiveMonitor(delay = 0) {
  if (!liveClients.size || liveMonitorTimer || liveMonitorRunning) return;
  liveMonitorTimer = setTimeout(runLiveMonitor, delay);
}

async function runLiveMonitor() {
  liveMonitorTimer = null;
  if (!liveClients.size || liveMonitorRunning) return;
  liveMonitorRunning = true;
  let delay = 5_000;
  try {
    const snapshot = await downloadSnapshot();
    const payload = JSON.stringify(snapshot);
    if (payload !== lastLiveDownloadPayload) {
      lastLiveDownloadPayload = payload;
      broadcastEvent('downloads', snapshot);
    }
    delay = snapshot.qbit?.state === 'ready' && snapshot.items.some(item => !item.complete) ? 2_000 : snapshot.qbit?.state === 'ready' ? 5_000 : 8_000;
  } catch {
    delay = 8_000;
  } finally {
    liveMonitorRunning = false;
    scheduleLiveMonitor(delay);
  }
}
...
function openEventStream(req, res) {
  ...
  writeEvent(res, 'ready', { ok: true, version: VERSION, connectedAt: Date.now() });
  writeEvent(res, 'playback', { ok: true, playback: currentPlayback });
  liveClients.add(res);
...
function languageFromSubtitle(name) {
  if (/^opensubtitles-\d+(?:\.en)?\.srt$/i.test(String(name))) return 'en';
...
function subtitleLabel(filePath) {
  const downloaded = path.basename(filePath).match(/^opensubtitles-(\d+)(?:\.en)?\.srt$/i);
  if (downloaded) return `English · OpenSubtitles ${downloaded[1]}`;
...
async function mediaCompatibility(filePath) {
  const stat = fs.statSync(filePath);
  const cacheKey = `${filePath}:${stat.size}:${stat.mtimeMs}`;
  if (mediaProbeCache.has(cacheKey)) return mediaProbeCache.get(cacheKey);
  let result = { state: 'unknown', container: path.extname(filePath).slice(1).toUpperCase(), videoCodec: '', audioCodec: '', width: 0, height: 0, hdr: false, note: 'Playback support will be checked on the TV.' };
  try {
    const output = await capture(mediaTool('ffprobe'), ['-v', 'error', '-show_format', '-show_streams', '-print_format', 'json', filePath], 15_000, 2_000_000);
    const probe = JSON.parse(output.toString());
    ...
    const knownVideo = new Set(['h264', 'hevc', 'vp9', 'av1', 'mpeg4']);
    const cautionAudio = new Set(['truehd', 'dts', 'dca']);
    const state = !video ? 'caution' : knownVideo.has(videoCodec) && !cautionAudio.has(audioCodec) ? 'ready' : 'caution';
    result = {
      state,
      container: String(probe.format?.format_name || path.extname(filePath).slice(1)).split(',')[0].toUpperCase(),
      videoCodec: videoCodec.toUpperCase(), audioCodec: audioCodec.toUpperCase(),
      width: Number(video?.width || 0), height: Number(video?.height || 0),
      hdr: /smpte2084|arib-std-b67/i.test(String(video?.color_transfer || '')),
      note: state === 'ready' ? '' : !video ? 'No playable video track was detected.' : 'The TV will verify this format when playback begins.',
    };
  } catch {}
  mediaProbeCache.set(cacheKey, result);
  while (mediaProbeCache.size > 200) mediaProbeCache.delete(mediaProbeCache.keys().next().value);
  return result;
}

async function previewFrame(file, atMs) {
  const bucket = Math.max(0, Math.min(86_400_000, Math.round(Number(atMs || 0) / 10_000) * 10_000));
  const key = `${file.absolutePath}:${bucket}`;
  if (previewCache.has(key)) return previewCache.get(key);
  const bytes = await capture(mediaTool('ffmpeg'), [
    '-hide_banner', '-loglevel', 'error', '-ss', String(bucket / 1000), '-i', file.absolutePath,
    '-frames:v', '1', '-vf', 'scale=360:-2', '-q:v', '6', '-f', 'image2pipe', '-vcodec', 'mjpeg', 'pipe:1',
  ], 12_000, 3_000_000);
  previewCache.set(key, bytes);
  while (previewCache.size > 80) previewCache.delete(previewCache.keys().next().value);
  return bytes;
}
...
async function playbackFiles(hash, refresh = false) {
  ...
  await Promise.all(videos.map(async file => { file.compatibility = await mediaCompatibility(file.absolutePath); }));
...
const subtitleDownloads = new Map();
function normalizedSubtitleTitle(value) {
  return String(value || '').normalize('NFKD').toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim();
}
function subtitleIdentity(file, manifest) {
  const episode = file.title.match(/\bS(\d{1,2})E(\d{1,3})\b/i) || file.title.match(/\b(\d{1,2})x(\d{1,3})\b/i);
  const title = episode ? cleanTitle(file.title.slice(0, episode.index)).trim() : (manifest.metadata?.title || cleanTitle(file.title).replace(/\s+\(?\b(?:19|20)\d{2}\b.*$/, '').trim());
  return { title, year: String(manifest.metadata?.year || file.title.match(/\b(?:19|20)\d{2}\b/)?.[0] || ''),
    tmdbId: manifest.metadata?.tmdbId || '', episode: episode ? { season: Number(episode[1]), number: Number(episode[2]) } : null };
}
async function subtitlesApi(route, options = {}) {
  if (!config.openSubtitlesApiKey) throw new Error('English subtitles need an OpenSubtitles API key in Phone Control Settings.');
  let response;
  try {
    response = await fetch('https://api.opensubtitles.com/api/v1/' + route, {
    ...
    const payload = await response.json();
    if (!response.ok) {
      if (response.status === 401 || response.status === 403) throw new Error('OpenSubtitles rejected the API key. Check it in Phone Control Settings.');
      if (response.status === 406 || response.status === 429) throw new Error('OpenSubtitles has reached its download or request limit. Please try later.');
      throw new Error(`OpenSubtitles is unavailable (${response.status}). Try again shortly.`);
    }
    return payload;
  } catch (error) {
    if (error.name === 'TimeoutError' || error.name === 'AbortError') throw new Error('Subtitle search timed out. Try again.');
    throw error;
  }
}
function subtitleCandidates(response, identity, fileTitle, identityQuery = false) {
  return (response.data || []).flatMap(item => {
    const a = item.attributes || {}, feature = a.feature_details || {};
    if (!['en', 'eng'].includes(String(a.language || '').toLowerCase()) || a.foreign_parts_only === true) return [];
    const exact = a.moviehash_match === true;
    const featureTitle = normalizedSubtitleTitle(feature.movie_name || feature.title);
    const titleMatches = featureTitle === normalizedSubtitleTitle(identity.title);
    const yearMatches = !identity.year || Math.abs(Number(feature.year) - Number(identity.year)) <= 1;
    const episodeMatches = identity.episode && Number(feature.season_number) === identity.episode.season && Number(feature.episode_number) === identity.episode.number;
    const matched = exact || (identity.episode
      ? Boolean(episodeMatches && (identityQuery || titleMatches))
      : Boolean((identityQuery || titleMatches) && yearMatches));
    if (!exact && identity.episode && !episodeMatches) return [];
    const releaseWords = normalizedSubtitleTitle(a.release).split(' ').filter(word => word.length > 2);
    const localWords = new Set(normalizedSubtitleTitle(fileTitle).split(' '));
    const releaseScore = releaseWords.filter(word => localWords.has(word)).length / Math.max(1, releaseWords.length);
    // Multi-file/CD releases need joining; don't silently attach only the first part.
    const files = a.files || [];
    return files.map(subtitle => ({
      id: Number(subtitle.file_id), language: 'en', label: exact ? 'Exact file match' : matched ? 'Matching title' : 'Check title',
      release: String(a.release || subtitle.file_name || 'English subtitles').slice(0, 150),
      rating: Number(a.ratings || 0), downloads: Number(a.download_count || 0),
      hearingImpaired: Boolean(a.hearing_impaired), exact, matched: matched && files.length === 1,
      releaseScore, trusted: Boolean(a.from_trusted), machineTranslated: Boolean(a.machine_translated || a.ai_translated),
    }));
  }).filter(item => Number.isSafeInteger(item.id) && item.id > 0);
}
async function searchSubtitles(hashValue, fileValue, language = 'en') {
  const { hash, file, manifest } = await playbackFile(hashValue, fileValue);
  const key = `${hash}:${file.id}:en`;
  const cached = subtitleSearchCache.get(key);
  if (cached && cached.expires > Date.now()) return cached.value;
  const identity = subtitleIdentity(file, manifest), moviehash = openSubtitlesHash(file.absolutePath);
  const common = { languages: 'en', foreign_parts_only: 'exclude', order_by: 'download_count', order_direction: 'desc' };
  const queries = [];
  if (moviehash) queries.push({ params: { ...common, moviehash, moviehash_match: 'only' }, identified: false });
  if (identity.tmdbId && (identity.episode || manifest.metadata?.mediaType !== 'TV')) {
    queries.push({ params: { ...common, ...(identity.episode
      ? { parent_tmdb_id: identity.tmdbId, season_number: identity.episode.season, episode_number: identity.episode.number, type: 'episode' }
      : { tmdb_id: identity.tmdbId, type: 'movie' }) }, identified: true });
  } else if (identity.title) {
    queries.push({ params: { ...common, query: identity.title, ...(identity.episode
      ? { season_number: identity.episode.season, episode_number: identity.episode.number, type: 'episode' }
      : { ...(identity.year ? { year: identity.year } : {}), type: 'movie' }) }, identified: false });
  }
  let failure;
  let candidates = (await Promise.all(queries.map(async query => {
    try { return subtitleCandidates(await subtitlesApi('subtitles?' + new URLSearchParams(query.params)), identity, file.title, query.identified); }
    catch (error) { failure = error; return []; }
  }))).flat();
  // An ID can have sparse coverage; a title/year search is a useful second route.
  if (!candidates.length && queries.some(query => query.identified) && identity.title) {
    try {
      const params = { ...common, query: identity.title, ...(identity.episode
        ? { type: 'episode', season_number: identity.episode.season, episode_number: identity.episode.number }
        : { type: 'movie', ...(identity.year ? { year: identity.year } : {}) }) };
      candidates = subtitleCandidates(await subtitlesApi('subtitles?' + new URLSearchParams(params)), identity, file.title);
    } catch (error) { failure = error; }
  }
  if (!candidates.length && failure) throw failure;
  const seen = new Set();
  const items = candidates.sort((a, b) => Number(b.exact) - Number(a.exact) || Number(b.matched) - Number(a.matched) || Number(a.machineTranslated) - Number(b.machineTranslated) || b.releaseScore - a.releaseScore || Number(b.trusted) - Number(a.trusted) || b.downloads - a.downloads)
    .filter(item => !seen.has(item.id) && seen.add(item.id)).slice(0, 30);
  const value = { ok: true, items };
  subtitleSearchCache.set(key, { value, expires: Date.now() + (items.length ? 600_000 : 60_000) });
  while (subtitleSearchCache.size > 100) subtitleSearchCache.delete(subtitleSearchCache.keys().next().value);
  return value;
}
function decodeSubtitleBytes(input) {
  let bytes = Buffer.from(input);
  if (!bytes.length || bytes.length > 10_000_000) throw new Error('The subtitle file was empty or too large.');
  // Fetch already decodes HTTP gzip, but leaves Content-Encoding on the response.
  // Only decompress bytes that actually contain a gzip file.
  if (bytes[0] === 0x1f && bytes[1] === 0x8b) {
    try { bytes = zlib.gunzipSync(bytes, { maxOutputLength: 10_000_000 }); }
    catch { throw new Error('The subtitle file was damaged. Try another match.'); }
  }
  let text;
  ...
  if (!/\d{1,2}:\d{2}:\d{2}[,.]\d{3}\s*-->\s*\d{1,2}:\d{2}:\d{2}[,.]\d{3}/.test(text) || /^\s*(<!doctype|<html|\{)/i.test(text)) {
    throw new Error('The provider returned an invalid subtitle. Try another match.');
  }
  return Buffer.from(text, 'utf8');
}
async function downloadSubtitle(hashValue, fileValue, subtitleId) {
  const id = Number(subtitleId);
  if (!Number.isSafeInteger(id) || id <= 0) throw new Error('Choose a subtitle from the English results.');
  const { hash, file } = await playbackFile(hashValue, fileValue);
  const directory = path.join(DATA_DIR, 'subtitles', hash, file.id);
  const target = path.join(directory, `opensubtitles-${id}.en.srt`);
  const key = `${hash}:${file.id}:${id}`;
  if (subtitleDownloads.has(key)) return subtitleDownloads.get(key);
  const job = (async () => {
    try {
      if (fs.statSync(target).size <= 10_000_000) {
        decodeSubtitleBytes(fs.readFileSync(target));
        return { ok: true, cached: true };
      }
    } catch {}
    const candidates = await searchSubtitles(hash, file.id);
    if (!candidates.items.some(item => item.id === id)) throw new Error('This English subtitle is no longer in the results. Search again.');
    const response = await subtitlesApi('download', { method: 'POST', body: JSON.stringify({ file_id: id, sub_format: 'srt' }) });
    if (!response.link) throw new Error('OpenSubtitles did not return that subtitle.');
    const link = new URL(response.link);
    if (link.protocol !== 'https:') throw new Error('The subtitle download link was not secure.');
    const download = await fetch(link, { signal: AbortSignal.timeout(20_000), headers: { 'user-agent': `SCREENGLO v${VERSION}` } });
    if (!download.ok) throw new Error(`Subtitle download failed (${download.status}). Try another match.`);
    const chunks = []; let size = 0;
    for await (const chunk of download.body) {
      size += chunk.length;
      if (size > 10_000_000) throw new Error('The subtitle file was too large.');
      chunks.push(chunk);
    }
    const bytes = decodeSubtitleBytes(Buffer.concat(chunks));
    fs.mkdirSync(directory, { recursive: true });
    fs.writeFileSync(target + '.tmp', bytes);
    fs.renameSync(target + '.tmp', target);
    playbackCache.delete(hash);
    return { ok: true };
  })().finally(() => subtitleDownloads.delete(key));
  subtitleDownloads.set(key, job);
  return job;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (route === 'api/config' && req.method === 'POST') {
      const data = await body(req); const normalizedLetterboxd = normalizeLetterboxdConfig(data); const submittedLists = String(data.letterboxdLists || '').split(/[\n,]+/).map(value => value.trim()).filter(Boolean);
      const recognizedLists = submittedLists.filter(value => normalizeLetterboxdConfig({ letterboxdLists: value }).letterboxdLists.length).length;
      const update = { qbitHost: String(data.qbitHost || '127.0.0.1').slice(0, 255), qbitPort: Number(data.qbitPort || 8080), qbitHttps: Boolean(data.qbitHttps), category: String(data.category || '').slice(0, 100), ...normalizedLetterboxd };
      ...
      saveConfig(update); return json(res, 200, { ok: true, config: publicConfig(), letterboxdSave: { submitted: submittedLists.length, saved: normalizedLetterboxd.letterboxdLists.length, rejected: Math.max(0, submittedLists.length - recognizedLists), duplicates: Math.max(0, recognizedLists - normalizedLetterboxd.letterboxdLists.length) } });
    }
    ...
    if (route === 'api/search' && req.method === 'POST') {
      ...
      const quality = ['any', '1080', '4k'].includes(data.quality) ? data.quality : 'any';
      const result = await searchCatalog(query, resolvedSort, quality); return json(res, 200, result);
    }
    if (route === 'api/metadata' && req.method === 'GET') {
      const metadata = await metadataFor({ tmdbId: String(url.searchParams.get('tmdbId') || '').slice(0, 16), title: String(url.searchParams.get('title') || '').slice(0, 240), year: url.searchParams.get('year') || '', mediaType: url.searchParams.get('type') || 'movie' });
    ...
    if (route === 'api/letterboxd/discover') {
      ...
      const limit = Math.max(1, Math.min(30, Number(url.searchParams.get('limit')) || 24));
      const offset = Math.max(0, Math.min(5000, Number(url.searchParams.get('offset')) || 0));
      const seed = String(url.searchParams.get('seed') || '').slice(0, 80);
      const watchedKeys = new Set(playbackHistory.map(item => `${normalizeHistoryTitle(item.title || item.seriesTitle)}|${String(item.year || '').slice(0, 4)}`));
      const watchedTitles = new Set(playbackHistory.map(item => normalizeHistoryTitle(item.title || item.seriesTitle)).filter(Boolean));
      const exclude = item => {
        const title = normalizeHistoryTitle(item.title);
        const key = `${title}|${String(item.year || '').slice(0, 4)}`;
        return watchedKeys.has(key) || watchedTitles.has(title);
      };
      return json(res, 200, { ok: true, ...(await letterboxd.discovery(filter, limit, seed, offset, exclude, url.searchParams.get('list') || '')) });
    }
    if (route === 'api/letterboxd/lists') {
      const catalog = await letterboxd.listCatalog({
        offset: url.searchParams.get('offset'), limit: url.searchParams.get('limit'), seed: url.searchParams.get('seed') || '',
    ...
    if (route === 'api/events') return openEventStream(req, res);
    if (route === 'api/history') return json(res, 200, { ok: true, items: playbackHistory.slice(0, 100) });
    if (route === 'api/history/watch' && req.method === 'POST') { const data = await body(req); return json(res, 200, { ok: true, item: recordWatched(data) }); }
    if (route === 'api/history/action' && req.method === 'POST') {
      const data = await body(req);
      if (data.action === 'clear') playbackHistory = [];
      else if (data.action === 'remove') playbackHistory = playbackHistory.filter(item => item.key !== String(data.key || ''));
      else return json(res, 400, { ok: false, message: 'That history action is unavailable.' });
      savePlaybackHistory(); broadcastEvent('history', { ok: true, items: playbackHistory.slice(0, 100) });
      return json(res, 200, { ok: true });
    }
    if (route === 'api/playback/state' && req.method === 'GET') return json(res, 200, { ok: true, playback: currentPlayback });
    if (route === 'api/playback/state' && req.method === 'POST') { const data = await body(req); return json(res, 200, { ok: true, playback: updatePlaybackState(data) }); }
    ...
    if (route === 'api/playback/preview') {
      const { file } = await playbackFile(url.searchParams.get('hash'), url.searchParams.get('file'));
      return send(res, 200, await previewFrame(file, url.searchParams.get('atMs')), 'image/jpeg', { 'cache-control': 'private, max-age=300' });
    }
    ...
    if (route === 'api/command' && req.method === 'POST') {
      const data = await body(req);
      const synced = data.action === 'exit' ? await tvPost('exit', {}) : await tvPost('command', { action: data.action });

...
function publicListUrl(value) {
  try {
    ...
    if (url.protocol !== 'https:' || !/^(?:www\.)?letterboxd\.com$/i.test(url.hostname)) return '';
...
class LetterboxdService {
  ...
  async discovery(filter = 'all', limit = 12, seed = '', offset = 0, exclude = null) {
    ...
    if (exclude) candidates = candidates.filter(item => !exclude(item));
    ...
    const ordered = orderRecommendations(candidates, filter, rotation);
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (route === 'api/letterboxd/discover') {
      ...
      return json(res, 200, { ok: true, ...(await letterboxd.discovery(filter, limit, seed, offset, exclude)) });
    }

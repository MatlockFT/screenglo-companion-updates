...
let detailReturn = 'results', discoverSwipeStart = null;
...
function resetDiscoverHome() {
  ...
  results.classList.add('hidden'); results.replaceChildren();
  recommendations.classList.remove('hidden'); searchStatus.textContent = '';
  $('#search-input').value = '';
}
function showView(id, reset = false) {
  activeView = id;
...
function memberBadge(name, state) {
  if (!state) return '';
  const label = state.watched ? `✓ ${name}${state.rating ? ` · ${state.rating}★` : ''}` : state.watchlist ? `♡ ${name} · saved` : `○ ${name}`;
  return `<span class="member-badge ${state.watched ? 'watched' : state.watchlist ? 'saved' : 'unwatched'}">${escape(label)}</span>`;
}
function letterboxdStrip(data) {
  if (!data?.available) return '';
  return `<div class="letterboxd-strip">${data.communityRating ? `<a class="letterboxd-score" href="${escape(data.url || '#')}" target="_blank" rel="noopener"><i></i><i></i><i></i> ★ ${escape(data.communityRating)}</a>` : ''}${memberBadge('Tyler', data.tyler)}${memberBadge('Gloria', data.gloria)}</div>`;
}
...
document.addEventListener('pointerdown', event => {
  discoverSwipeStart = activeView === 'discover' && !detail.classList.contains('hidden') && event.isPrimary && event.clientX <= 48
    ? { x: event.clientX, y: event.clientY }
    : null;
});
document.addEventListener('pointerup', event => {
  if (!discoverSwipeStart || !event.isPrimary) return;
  const dx = event.clientX - discoverSwipeStart.x, dy = Math.abs(event.clientY - discoverSwipeStart.y);
  discoverSwipeStart = null;
  if (dx >= 72 && dy <= 64) { buzz(); closeDetail(); }
});
...
async function loadDownloads(silent = false) {
  ...
  try {
    ...
    status.textContent = data.items.length ? `LIVE · ${data.items.length} download${data.items.length === 1 ? '' : 's'} · newest first` : 'No downloads yet';
    renderDownloads(data.items);
    if (activeView === 'downloads') scheduleDownloads(data.items.some(item => !item.complete) ? 2000 : 8000);
  } catch (error) {
    downloadFailures += 1;
...
async function loadForUs(force = false) {
  ...
  try {
    ...
    const data = await api(`letterboxd/discover?filter=${encodeURIComponent(letterboxdFilter)}`);
...
async function loadSettings() {
  try {
    ...
    $('#companion-version').textContent = `Version ${data.version} · encrypted locally`; checkUpdate(true);
...
async function saveSettings() {
  ...
  const payload = {
    ...
    qbitApiKey: $('#qbit-key').value.trim(), tmdbToken: $('#tmdb-token').value.trim(), category: $('#category').value.trim(),
  ...
  try {
    ...
    status.textContent = `Connected to qBittorrent ${test.version} ✓`; $('#qbit-key').value = ''; $('#tmdb-token').value = '';

...
async function applyNavigation(state) {
  applyingNavigation = true;
  try {
    showView(state.view, false, false);
...
function navigateBack() {
  ...
  navigationIndex -= 1; buzz(); applyNavigation(navigationStack[navigationIndex]);
}
function navigateForward() {
  if (navigationIndex >= navigationStack.length - 1) return;
  navigationIndex += 1; buzz(); applyNavigation(navigationStack[navigationIndex]);
}
function showView(id, reset = false, record = true) {
  activeView = id;

...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/pair' || url.pathname === '/health') return json(res, 200, { ok: true, url: baseUrl(), version: VERSION, signedUpdates: true, configured: Boolean(config.qbitApiKey) });
    ...
    if (route === 'api/state') return json(res, 200, { ok: true, tv: await tvState(), configured: Boolean(config.qbitApiKey), companion: { version: VERSION, ready: true, tvLastSeenAt: tv.lastSeen || null } });
    ...
    if (route === 'api/letterboxd/discover') {
      ...
      return json(res, 200, { ok: true, ...(await letterboxd.discovery(filter)) });
    }

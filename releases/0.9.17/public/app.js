...
function showView(id) {
  ...
  if (id === 'discover') loadForUs();
  if (id === 'downloads') { loadDownloads(); scheduleDownloads(2000); } else clearTimeout(downloadsTimer);
  if (id === 'settings') loadSettings();
  scrollTo({ top: 0, behavior: 'smooth' });
}
$$('[data-view]').forEach(button => button.addEventListener('click', () => { buzz(); showView(button.dataset.view); }));
...
async function openDetail(id) {
  ...
  try {
    ...
    detail.innerHTML = `
      <div class="detail-head"><button class="close">← RESULTS</button></div>
    ...
    detail.querySelector('.close').addEventListener('click', () => { detail.classList.add('hidden'); results.classList.remove('hidden'); searchStatus.textContent = ''; });

...
class LetterboxdService {
  ...
  async tmdbCollection(collection, limit, offset, exclude = null) {
    ...
    let cached = this.tmdbCollections.get(cacheKey);
    const desired = Math.min(200, Math.max(offset + limit + 30, 60));
    if (!cached || Date.now() - cached.at >= TMDB_COLLECTION_TTL || cached.items.length < desired) {
      const items = [];
      let total = 0;
      for (let page = 1; page <= Math.min(10, Math.ceil(desired / 20)); page++) {
        ...
        try {
          ...
          const payload = await response.json(); total = Math.min(10 * 20, Number(payload.total_results || 0));
      ...
      cached = { at: Date.now(), items: items.filter((item, index, all) => all.findIndex(candidate => candidate.tmdbId === item.tmdbId) === index), total };
    ...
    return { status: this.status(), total: available.length, offset, hasMore: offset + selected.length < available.length, items: selected };
  }

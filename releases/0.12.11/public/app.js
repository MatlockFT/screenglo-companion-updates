...
const liveChannels = {
  ...
  sports: { name: 'Red Bull TV', url: 'https://rbmn-live.akamaized.net/hls/live/590964/BoRB-AT/master.m3u8' },
};
$$('[data-live-channel]').forEach(button => button.addEventListener('click', async () => {
  const channel = liveChannels[button.dataset.liveChannel], video = $('#live-video');
  stopLiveChannel();
  ...
  try { await video.play(); $('#live-status').textContent = channel.name + ' · LIVE'; }
  catch { $('#live-status').textContent = 'Tap Play to start, or choose the channel again to retry.'; }
}));
$('#live-video').addEventListener('error', () => { $('#live-status').textContent = 'Channel unavailable. Choose the channel again to retry.'; });
document.addEventListener('visibilitychange', () => { if (document.hidden) $('#live-video').pause(); });

...
function Set-RunningStatus {
  @{ state = 'running'; version = '0.11.0'; message = 'SCREENGLO Companion and tray are running.'; startedAt = (Get-Date).ToString('o') } |
...
try {
  try {
    ...
    $notifyIcon.Text = 'SCREENGLO Companion - Needs attention'
    @{ state = 'failed'; version = '0.11.0'; message = $_.Exception.Message; failedAt = (Get-Date).ToString('o') } |
      ConvertTo-Json | Set-Content -Encoding UTF8 $statusFile

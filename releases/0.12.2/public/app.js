...
async function loadForUs(force = false) {
  ...
  try {
    ...
    const data = await api(`letterboxd/discover?filter=${encodeURIComponent(letterboxdFilter)}&limit=24`);
    letterboxdItems = data.items;
    status.textContent = data.items.length ? `${data.items.length} unwatched picks${data.status?.degraded ? ' · using saved data for one list' : ''}` : data.status?.syncing ? 'First sync is still running…' : 'No matching unwatched picks yet.';
    list.innerHTML = data.items.slice(0, 24).map(item => `<article class="letterboxd-film" data-recommendation="${escape(recommendationId(item))}">

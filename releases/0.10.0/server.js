...
async function qbit(endpoint, options = {}) {
  ...
  let response;
  try {
    response = await fetchWithTimeout(qbitUrl(endpoint), { ...options, headers: { authorization: `Bearer ${config.qbitApiKey}`, ...(options.headers || {}) } }, 10_000);
  } catch (error) {
...
async function playbackFiles(hash, refresh = false) {
  ...
  const result = { hash, title: String(properties.name || videos[0].title), files: videos, cachedAt: Date.now() };
...
function publicPlayback(manifest) {
  return {
    ok: true, hash: manifest.hash, title: manifest.title,

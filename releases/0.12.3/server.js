...
class LetterboxdService {
  ...
  async discovery(filter = 'all', limit = 12, seed = '') {
    ...
    const selected = orderRecommendations(candidates, filter, rotation).slice(0, Math.max(1, Math.min(60, Number(limit) || 12)));
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (route === 'api/letterboxd/discover') {
      ...
      const historyAllowance = Math.min(12, playbackHistory.length);
      const discovery = await letterboxd.discovery(filter, Math.min(60, limit + historyAllowance), seed);
      ...
      return json(res, 200, { ok: true, ...discovery, items });
    }

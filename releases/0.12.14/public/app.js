...
let japanLoadedAt = 0, japanLoading = false, livePlayRequest = 0;
async function loadJapanChannels() {
  if (japanLoading || (japanLoadedAt && Date.now() - japanLoadedAt < 3600000)) return;
  japanLoading = true; $('#japan-status').textContent = 'Loading channels…';
  try {
    const response = await fetch('https://iptv-org.github.io/iptv/countries/jp.m3u', { signal: AbortSignal.timeout(12000) });
    ...
    channels.slice(0, 500).forEach(channel => {
      ...
      button.type = 'button'; button.append(document.createTextNode(channel.name)); small.textContent = channel.category; button.append(small);
...
async function playLiveChannel(channel) {
  ...
  try { await video.play(); if (request === livePlayRequest) $('#live-status').textContent = channel.name + ' · LIVE'; }
  catch { if (request === livePlayRequest) $('#live-status').textContent = 'Tap Play to start, or choose the channel again to retry.'; }
}
$$('[data-live-channel]').forEach(button => button.addEventListener('click', () => playLiveChannel(liveChannels[button.dataset.liveChannel])));
